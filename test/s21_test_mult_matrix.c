...
START_TEST(mult_matrix) {
  ...
  matrix_t res = {0};
  ck_assert_int_eq(s21_mult_matrix(&m, &mtx, &res), TRULY);
...
START_TEST(mult_matrix3) {
  ...
  matrix_t res = {0};
  ck_assert_int_eq(s21_mult_matrix(&m, &mtx, &res), CALCULATION);
...
Suite *test_mult_matrix(void) {
  Suite *s = suite_create("TEST MULT MATRIX");
  TCase *tc = tcase_create("CASE TEST MULT MATRIX");
