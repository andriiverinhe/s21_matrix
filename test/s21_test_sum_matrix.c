...
START_TEST(sum_matrix) {
  ...
  ck_assert_int_eq(s21_sum_matrix(&m, &mtx, &res), TRULY);
...
START_TEST(sum_matrix1) {
  ...
  matrix_t res = {0};
  ck_assert_int_eq(s21_sum_matrix(&m, &mtx, &res), CALCULATION);
  s21_remove_matrix(&m);
  s21_remove_matrix(&mtx);
  s21_remove_matrix(&res);
}
...
Suite *test_sum_matrix(void) {
  Suite *s = suite_create("TEST SUM MATRIX");
  TCase *tc = tcase_create("CASE TEST SUM MATRIX");
