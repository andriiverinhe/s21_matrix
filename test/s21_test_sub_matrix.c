...
START_TEST(sub_matrix) {
  ...
  matrix_t res = {0};
  ck_assert_int_eq(s21_sub_matrix(&m, &mtx, &res), TRULY);
...
START_TEST(sub_matrix1) {
  ...
  matrix_t res = {0};
  ck_assert_int_eq(s21_sub_matrix(&m, &mtx, &res), CALCULATION);
  s21_remove_matrix(&m);
  s21_remove_matrix(&mtx);
  s21_remove_matrix(&res);
}
...
Suite *test_sub_matrix(void) {
  Suite *s = suite_create("TEST SUB MATRIX");
  TCase *tc = tcase_create("CASE TEST SUB MATRIX");
