...
START_TEST(test_one_by_one) {
  ...
  if (codec) {
    ...
    ck_assert_int_eq(code, CALCULATION);
  }
    s21_remove_matrix(&m);
}
END_TEST
...
Suite *test_calc_complements(void) {
  Suite *s = suite_create("TEST CALC COMPLEMENTS");
  TCase *tc = tcase_create("CASE TEST CALC COMPLEMENTS");
